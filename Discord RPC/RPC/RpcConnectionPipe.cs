...
{
	internal class RpcConnectionPipe
	{
		/// <summary>
		...
		public RpcConnectionPipe(string applicationID, int process)
		{
		...
		private bool Connect()
		{
			...
			if (!WriteFrame(Opcode.Handshake, new Handshake()
			{
				ClientID = this.ApplicationID,
				Version = VERSION
			}))
			{
				LogDebug("Failed to write handshake");
				return false;
			}
		...
		private bool WriteFrame(Opcode opcode, object obj)
		{
			...
			//Send to the pipe
			bool success = pipe.WriteFrame(frame);
			LogDebug("Success: {0}, Opcode: {1}", success, opcode);
