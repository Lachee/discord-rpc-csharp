...
public class DiscordNativeNamedPipe : INamedPipeClient
{
    ...
    /// </summary>
    public bool IsConnected { get { return DiscordNativePipeExterns.IsConnected(); } }
    ...
    /// </summary>
    internal DiscordNativePipeExterns.PipeReadError LastError { get { return _lasterr; } }
    private DiscordNativePipeExterns.PipeReadError _lasterr = DiscordNativePipeExterns.PipeReadError.None;
    ...
    private bool AttemptConnection(int pipe)
    {
        ...
        uint err = DiscordNativePipeExterns.Open(pipename);
    ...
    public bool ReadFrame(out PipeFrame frame)
    {
        ...
        try
        {
            bytesRead = DiscordNativePipeExterns.ReadFrame(pnt, _buffer.Length);
        ...
        if (bytesRead <= 0)
        {
            ...
            _lasterr = DiscordNativePipeExterns.PipeReadError.ReadEmptyMessage;

            //A error actively occured. If it is 0 we just read no bytes.
            if (bytesRead < 0)
            {
                //We have a pretty bad error, we will log it for prosperity. 
                _lasterr = (DiscordNativePipeExterns.PipeReadError)bytesRead;
    ...
    public bool WriteFrame(PipeFrame frame)
    {
        ...
        using (MemoryStream stream = new MemoryStream())
        {
            ...
            try
            {
                ...
                return DiscordNativePipeExterns.WriteFrame(pnt, bytes.Length);
            }
            finally
            {
                Marshal.FreeHGlobal(pnt);
    ...
    public void Close()
    {
        DiscordNativePipeExterns.Close();
    }
...
internal static class DiscordNativePipeExterns
{
